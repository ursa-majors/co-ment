    ...
    evaluate() {
      ...
      if (value.length >= this.minChars && this._list.length > 0) {
        ...
        this.suggestions = this._list
				.map(item => new Suggestion(me.data(item, value)))
				.filter(item => me.filter(item, value));
...
// Static methods/properties
  ...
  _.ITEM = function (text, input) {
    const html = input.trim() === '' ? text : text.replace(RegExp($.regExpEscape(input.trim()), 'gi'), '<mark>$&</mark>');
    return $.create('li', {
      innerHTML: html,
      'aria-selected': 'false',
    });
